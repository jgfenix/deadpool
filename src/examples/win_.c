...
static void//inwin mode 'on'
   b_inw_cb(void *data, Evas_Object *previous_win, void *event_info)
   {
    
    Evas_Object  *h_box, *ic, *b_back, *bt ;
    
    _win = elm_object_top_widget_get(previous_win);//get the previous window as parameter
      
    in_win = elm_win_inwin_add(_win); 
    evas_object_show(in_win);//OR elm_win_inwin_activate(in_win);
        
    h_box = elm_box_add(_win);
    elm_box_horizontal_set(h_box, EINA_TRUE);
    evas_object_size_hint_weight_set(h_box, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND); 
    elm_win_resize_object_add(_win, h_box);
    elm_box_padding_set(h_box, 1, 1);
    evas_object_show(h_box);

    b_back = elm_button_add(h_box);
    elm_object_text_set(b_back, " back ");
    elm_box_pack_end(h_box, b_back);
    ...
    bt = elm_button_add(h_box);
    elm_object_text_set(bt, "chat ");
    elm_object_part_content_set(bt, "icon", ic);
    elm_box_pack_end(h_box, bt);
    evas_object_show(bt);
    ...
    elm_win_inwin_content_set(in_win, h_box);  
   }


void//make 'buttons' with File and print the file in terminal
   _file_chosen(void *data, Evas_Object *obj, void *event_info)
  {  
   Evas_Object *bt;
   const char *file = event_info;
   
   //_win = elm_object_top_widget_get(obj);
...
void//naviframe mode
  ...
  {
   Evas_Object *nv = data, *ic, *fs_bt, *en, *h_box, *b_n;
   

   h_box = elm_box_add(nv);
   elm_box_horizontal_set(h_box, EINA_FALSE);
   evas_object_size_hint_weight_set(h_box, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND); 
   elm_win_resize_object_add(nv, h_box);
   evas_object_show(h_box);
...
{
   Evas_Object *win, *nv, *box, *t_box, *ic,  *b_n, *b_exit, *b_app ;
...
/*----------------------------------------------------------------------------------*/
   ...
   t_box = elm_box_add(nv);
   elm_box_horizontal_set(t_box, EINA_TRUE);
   elm_box_homogeneous_set(t_box, EINA_TRUE);
   evas_object_size_hint_weight_set(t_box, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND); 
   elm_win_resize_object_add(win, t_box);
   evas_object_show(t_box);
   ...
   elm_naviframe_item_push(nv, NULL, NULL, box, b_app, NULL);
